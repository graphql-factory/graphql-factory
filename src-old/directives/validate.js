...
export default {
  name: 'validate',
  description:
    'Validates a field using a custom validator function. ' +
  ...
  args: {
    validator: {
      type: 'String!',
      description:
        'Reference to the name of the validator function ' +
  ...
  before(source, args, context, info) {
    switch (info.location) {
      ...
      case DirectiveLocation.INPUT_FIELD_DEFINITION:
        ...
        assert(
          validator(source),
          'Validation failed on argument "' +
            info.attachInfo.argName +
            '" of field "' +
            info.attachInfo.parentField.name +
            '" of type "' +
  ...
  after(source, args, context, info) {
    switch (info.location) {
