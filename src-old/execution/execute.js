...
import { completeTrace, emit } from './instrumentation';
  ...
  const { resolveBefore, resolveAfter } = getDirectiveContext(
    ...
    [
      ...
      },
      {
        ...
        parentType: getOperationRootType(schema, context.operation),
      },
    ],
    tracing,
    undefined,
    ...
    .then(() => {
      if (resolveBefore.length) {
        return promiseReduce(resolveBefore, (accum, dirInfo) => {
          ...
          return resolve(rootValue, args, contextValue, info);
        });
      }
    })
    .then(() => {
...
function buildResponse(
  ...
  tracing.resolverDuration = _.reduce(
    tracing.execution.resolvers,
    (total, exec) => {
      return total + exec.duration;
    },
    0,
  );
  tracing.overheadDuration =
    tracing.duration -
    tracing.resolverDuration -
    tracing.parsing.duration -
    tracing.validation.duration;

  if (context.errors.length) {
    emit(context, 'error', context.errors);
...
function resolveField(
  ...
  if (
    ...
    if (typeof property === 'function') {
      resolver = property.name || 'defaultFieldResolver';
    }
  }
...
function collectAndExecuteSubfields(
  ...
  const { fields, fragments } = collectSubfields(
    exeContext,
    returnType,
    fieldNodes,
  );
