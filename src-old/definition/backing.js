...
function assertFn(fn: any, name: string) {
  return fn ? assert(_.isFunction(fn), `${name} must be a function`) : false;
}
...
function assertObj(obj: any, name: string) {
  return obj ? assert(_.isObjectLike(obj), `${name} must be an object`) : false;
}
...
function validate(backing: any) {
  ...
  forEach(
    ...
    config => {
      ...
      valid = valid || assertFn(config.before, 'before');
      valid = valid || assertFn(config.after, 'after');
      valid = valid || assertFn(config.build, 'build');
...
function exportResolverMap(backing: SchemaBacking) {
  ...
  return reduce(
    ...
    (rmap, config, name) => {
      ...
      return _.keys(fields).length ? _.merge(rmap, { [name]: fields }) : rmap;
    },
    {},
    true,
  );
...
export class DirectiveBacking extends TypeBacking {
  ...
  before(func: () => ?mixed) {
    assert(_.isFunction(func), 'before requires a function');
    _.set(this._backing, ['_directives', this._name, 'before'], func);
    return this;
  }
  after(func: () => ?mixed) {
    assert(_.isFunction(func), 'after requires a function');
    _.set(this._backing, ['_directives', this._name, 'after'], func);
    return this;
  }
  build(func: () => ?mixed) {
    assert(_.isFunction(func), 'build requires a function');
    _.set(this._backing, ['_directives', this._name, 'build'], func);
    return this;
  }
}
