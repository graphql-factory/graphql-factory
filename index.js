 ...
 * Build: `lodash modularize exports="es" include="assign,capitalize,castArray,filter,find,forEach,get,has,identity,includes,isArray,isFunction,isNumber,isObject,isString,keys,map,noop,omit,reduce,set,union,uniq,values,without" -o src/common/lodash`
...
var cloneableTags = {};
cloneableTags[argsTag$3] = cloneableTags[arrayTag$2] = cloneableTags[arrayBufferTag$2] = cloneableTags[dataViewTag$3] = cloneableTags[boolTag$2] = cloneableTags[dateTag$2] = cloneableTags[float32Tag$1] = cloneableTags[float64Tag$1] = cloneableTags[int8Tag$1] = cloneableTags[int16Tag$1] = cloneableTags[int32Tag$1] = cloneableTags[mapTag$3] = cloneableTags[numberTag$3] = cloneableTags[objectTag$3] = cloneableTags[regexpTag$2] = cloneableTags[setTag$3] = cloneableTags[stringTag$3] = cloneableTags[symbolTag$2] = cloneableTags[uint8Tag$1] = cloneableTags[uint8ClampedTag$1] = cloneableTags[uint16Tag$1] = cloneableTags[uint32Tag$1] = true;
 ...
 */
var Enum = function Enum(config, options) {
  var _this = this;
  ...
  _.forEach(_config, function (value, key) {
    if (_this.hasKey(key)) {
      ...
      return false;
    } else if (_this.hasValue(value) && !_.get(options, 'allowDuplicates')) {
...
var GraphQLFactoryMiddleware = function GraphQLFactoryMiddleware(type, resolve, options) {
  ...
  var _ref = isHash(options) ? options : {},
  ...
  this.name = stringValue(name) ? name : null;
...
function parseAST(nodes) {
  if (Array.isArray(nodes)) {
    return nodes.reduce(function (obj, node) {
      ...
      switch (kind) {
        case graphql.Kind.DIRECTIVE:
          obj[name.value] = !Array.isArray(args) || !args.length ? true : parseAST(args);
          break;

        default:
          obj[name.value] = value.kind === graphql.Kind.LIST ? value.values.map(parseAST) : parseAST(value);
          break;
      }

      return obj;
    }, {});
  } else if ((typeof nodes === 'undefined' ? 'undefined' : _typeof(nodes)) === 'object' && nodes !== null) {
    var kind = nodes.kind,
        value = nodes.value,
        fields = nodes.fields;
 ...
 * @returns {Array}
 */
function executionPath(info) {
  var path = [];
  var _info$path = info.path,
      prev = _info$path.prev,
      key = _info$path.key;
 ...
 * @param info
 */
function executionInfo(info) {
  var operation = info.operation,
      schema = info.schema;

  var execInfo = [schema];
  var path = executionPath(info);
  var operationType = operation.operation;
  var currentType = schema['_' + operationType + 'Type'];
  ...
  try {
    for (var _iterator = path[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var field = _.get(currentType, ['_fields', fieldName]);
      execInfo.push(field);
 ...
 * @returns {*}
 */
function filterDirectives(info, source) {
  if (!source) return [];
  var _info$schema = info.schema,
      _directives = _info$schema._directives,
      _typeMap = _info$schema._typeMap;
  var directives = source.directives,
      kind = source.kind,
      args = source.arguments,
      type = source.type;

  var loc = KindLocation[kind];

  // reduce the args first
  var results = _.reduce(args, function (result, arg) {
    return result.concat(filterDirectives(info, arg));
  }, []) || [];

  // if there is a type, check it for directives
  if (type) {
    var _$get = _.get(_typeMap, baseName(type), {}),
        astNode = _$get.astNode;

    results = results.concat(filterDirectives(info, astNode));
  }

  // reduce the directives to the allowed ones
  _.reduce(directives, function (result, _ref) {
    var name = _ref.name,
        _args = _ref.arguments;

    // find the current directive
    var directive = _.find(_directives, function (d) {
 ...
 * @param info
 */
function reduceDirectives(info) {
  return _.reduce(executionInfo(info).reverse(), function (result, nfo) {
...
function processMiddleware(schema, backing, definition, params) {
  var _params = slicedToArray(params, 4),
      source = _params[0],
      args = _params[1],
      context = _params[2],
      info = _params[3];
  ...
  var directives = reduceDirectives(info);
...
var GraphQLFactoryBacking = function () {
  ...
  createClass(GraphQLFactoryBacking, [{
    ...
    value: function hydrateSchema(schema, definition) {
      ...
      _.forEach(this.backing, function (typeBacking, typeName) {
        if (err) return false;
      ...
      _.forEach(schema._directives, function (directive) {
        ...
        var _$get = _.get(_this3.backing, '@' + name, {}),
            before = _$get.before,
            after = _$get.after,
            error = _$get.error;
    ...
    value: function Directive(name, directive) {
      var directiveBacking = {};
      var _before = directive._before,
          _after = directive._after,
          _error = directive._error;
...
var GraphQLFactoryDefinitionTranslator = function () {
  ...
  createClass(GraphQLFactoryDefinitionTranslator, [{
    ...
    value: function _Directive(definition, directiveName) {
      ...
      var _loc = _locations.length ? _locations.join(' | ') : _.values(graphql.DirectiveLocation).join(' | ');
