...
export default {
  ...
  args: {
    isTypeOf: {
      type: 'String',
      description: 'A reference to an isTypeOf function. Can only be used ' +
      'on a GraphQLObjectType'
    },
    returnType: {
      type: 'String',
      description: 'A reference to a returnType function. Can only be ' +
      'used on a GraphQLInterfaceType or GraphQLUnionType'
    },
    serialize: {
      type: 'String',
      description: 'A reference to a serialize function. Can only be ' +
      'used on a GraphQLScalarType'
    },
    parseValue: {
      type: 'String',
      description: 'A reference to a parseValue function. Can only be ' +
      'used on a GraphQLScalarType'
    },
    parseLiteral: {
      type: 'String',
      description: 'A reference to a parseLiteral function. Can only be ' +
      'used on a GraphQLScalarType'
    }
  },
  beforeBuild(source, args, context, info) {
    try {
      forEach(args, (name, type) => {
        const path = [ 'definition', 'functions', name ];
        const fn = _.get(info, path);
        assert(
          _.isFunction(fn),
          '@typeConfig ' + type + ' function ' + name +
        ...
        switch (type) {
          case 'isTypeOf':
            assert(
              source.type === 'Object',
              '@typeConfig directive argument ' + type +
